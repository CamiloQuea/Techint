...
on(document, 'submit', '#modalTabForm', async e => {
    ...
    if (tipo === 'crear') {
        ...
        const data = {
            ...
            proyecto: selectModal['proyecto_id'].value
...
async function deleteBtn(btn) {
    ...
    const proyecto_id = elemento[3].innerHTML;

    if (confirm('ESTA SEGURO DE ELIMINAR?')) {

        await deleteItem(conductor_id, proyecto_id);
...
const createItem = async (body) => {
    ...
    const data = await fetch('/api/conductor/proyecto', {
        method: 'POST',
        headers: {
...
const deleteItem = async (conductor, proyecto) => {

    const data = await fetch(`/api/conductor/proyecto?conductor=${conductor}&proyecto=${proyecto}`, {
        method: 'DELETE'
    });
...
const updateTable = async () => {
    ...
    if (datajson.length === 0) {
        ...
        return `<tr>
                    <td colspan="7">No hay usuario con proyectos</td>
                </tr>`;
    }
    ;
    ...
    for (let i = 0; i < datajson.length; i++) {
        ...
        contentUpdated += `<tr class="align-middle">
                                ...
                                <td>${datajson[i].proyecto_id}</td>
                                <td>${datajson[i].proyecto_codigo}</td>
                                <td>${datajson[i].proyecto_nombre}</td>
...
const updateProyectoSelect = async () => {
    ...
    const selectrol = document.getElementById('select_proyecto');

    if (datajson.length === 0) {

        return `<option value="">NO HAY PROYECTOS</option>`;
    }
    ;

    let contentUpdated = `<option value=""> SELECCIONE PROYECTO</option>`;

    for (let i = 0; i < datajson.length; i++) {

        contentUpdated += `<option value="${datajson[i].id}"> ${datajson[i].codigo} - ${datajson[i].detalle}</option>`;
