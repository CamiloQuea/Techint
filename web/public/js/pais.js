...
const editBtn = async(btn) => {
    ...
    const iso = elemento[1].innerHTML;
    
    const nombre = elemento[2].innerHTML;

    inputsModal['id'].value = id;

    inputsModal['iso'].value = iso;
...
on(document, 'click', '#addBtn', e => {
    ...
    modaltitle.innerHTML = 'AGREGAR PAIS';
...
const deleteItem = async (id) => {
    ...
    const data = await fetch(`/api/pais?id=${id}`, {
        method: 'DELETE'
    });
